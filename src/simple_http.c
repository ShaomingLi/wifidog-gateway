...
/**
 * Perform an HTTP request, caller frees both request and response,
 * NULL returned on error. We don't want to overwrite http_get, so we
 * use a new name http_request
 * @param sockfd Socket to use, already connected
 * @param req Request to send, fully formatted.
 * @return char Response as a string
 */
char *
http_request(const int sockfd, const char *req)
{
    ssize_t numbytes;
    int done, nfds;
    fd_set readfds;
    ...
    debug(LOG_DEBUG, "Sending HTTP request to auth server: [%s]\n", req);
    size_t bytesLeft = reqlen;
    const char *startPos = req;
    while(bytesLeft > 0)
    {
        numbytes = send(sockfd, startPos, bytesLeft, 0);
        if (numbytes < 0) {
            debug(LOG_ERR, "send failed: %s", strerror(errno));
            goto error;
        }
    ...
    if (bytesLeft != 0) {
            debug(LOG_ERR, "send failed: only %d bytes out of %d bytes sent!", reqlen - bytesLeft, reqlen);
            goto error;
    }
